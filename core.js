(function(root){
  ...
  function apply(data,cmd,actor){
    ...
    if(cmd.type==='seed'){
      ...
      for(const target of cmd.targets){
        const p=db.products[target.id];if(!p)continue;
        matched++;const targetStock=int(target.stock,'Original catalogue stock',0,1000000),delta=targetStock-p.stock;
        if(delta!==0){p.stock=targetStock;p.updatedAt=cmd.at;movement(p.id,delta,'Restock to original SUCCESS26 catalogue quantity');}
      }
      requireThat(matched>0,'None of the SUCCESS26 catalogue products exist in this inventory.');
  ...
  const api={blank,normalize,apply,summary,shiftSummary,dateKey,salesCsv};if(typeof module!=='undefined')module.exports=api;else root.StockFlowCore=api;
})(globalThis);
